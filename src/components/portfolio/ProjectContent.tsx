...
export const ProjectContentVolvo: React.FC<ProjectContentProps> = ({
  project,
  slugify,
}) => {
  return (
...
export const ProjectContentDefault: React.FC<ProjectContentProps> = ({
  project,
  slugify,
}) => {
  return (
        ...
        {project.toc?.map((section, index) => (
          <React.Fragment key={index}>
            ...
            <div className="col-span-1 md:col-span-8 md:col-start-1 mb-16">
              <p className="text-base leading-7 font-light text-foreground/80 text-pretty">
                {/* For now, just repeating the description or generic content since we don't have rich text for other projects yet. */}
                {index === 0 ? project.content : project.description}
              </p>
              
              {/* If there are gallery images, distribute them */}
              {project.gallery && project.gallery[index] && (
